...
db.students.find({birth_year: {$gte: 1993,$lt: 1994}})

//4. Busqueu els estudiants de gènere masculí nascuts a l’any 1993
db.students.find({birth_year: {$gte: 1993,$lt: 1994}, gender: "H"})
...
db.students.find({ birth_year: { $ne: [1970, 1980, 1990] } })
...
db.students.find({phone_aux: { $exists: true, $ne: " "}})
...
db.students.find({phone_aux: { $exists: true, $ne: ""} , lastname2:""} )

//19. Busqueu els estudiants que tinguin un email que acabi en .net
db.students.find({ email: { $regex: /.net$/, $options: 'i'}})
...
db.students.find({firstname: /.[aeiouAEIOU].[aeiouAEIOU].[aeiouAEIOU].[aeiouAEIOU].*/})

//23. Busqueu els estudiants que tinguin un dni que comenci per lletra
db.students.find({ dni: { $regex: /^[a-z]/, $options: 'i'}})
...
db.students.find({ phone: { $regex: /^[6][2][2]/, $options: 'i'}})